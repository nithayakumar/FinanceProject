              ...
              {projections.map((p) => {
                ...
                return (
                  <tr key={p.year} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right text-blue-600">{formatSmart(isPV ? p.cashPV : p.cash)}</td>
                    <td className="py-3 px-4 text-right text-purple-600">{formatSmart(isPV ? p.totalInvestmentValuePV : p.totalInvestmentValue)}</td>
                    <td className="py-3 px-4 text-right text-emerald-600">{formatSmart(isPV ? p.retirement401kValuePV : p.retirement401kValue)}</td>
                    <td className="py-3 px-4 text-right text-amber-600">{formatSmart(isPV ? p.homeEquityPV : p.homeEquity)}</td>
        ...
        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
          <div className="flex flex-wrap gap-6 text-xs text-gray-500">
            ...
            <div className="flex items-center gap-2">
              ...
              <span><strong>Net Worth:</strong> Cash + Investments + 401k + Home Equity</span>
            </div>
          </div>
        </div>
      </div>
...
function NetWorthBreakdownTable({ projections, isPV, fmt, tableViewMode, setTableViewMode, expenseProjections, incomeProjections, investmentsData }) {
  ...
  const breakdownData = projections.map((p, index) => {
    ...
    const operatingExpenses = expenses // Base living expenses
    const totalOperatingOutflows = operatingExpenses + annualPropertyCosts + annualMortgageInterest
